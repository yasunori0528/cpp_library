...
void calc_cmp1(vector<int> v){
    ...
    do{
        ...
        else if (MAX_CMP.abs <= cmp.abs && cmp.pqkable() <= 2){
            MAX_CMP = cmp;
            cmp.print();
        }
...
void calc2_sub(power pw){
    for(int i = 1; i <= m-pw.digit; i++){
        ...
        for(composite cmp : CMP_list_small[i]){
            ...
            if (MAX_CMP.abs <= cmp.abs && cmp.pqkable() <= 2){
                MAX_CMP = cmp;
                cmp.print();
            }
        }
...
void calc2(){
    ...
    //n+1桁以上の素数を順に探索
    MAX_N = pow_n((mpz_class)10,(mpz_class)(72-n-1));
...
int main(){
    ...
    /*---------------- ----------------*/
    cin >> p_start;
    MAX_CMP.abs = pow_n((mpz_class)3,(mpz_class)8)*pow_n((mpz_class)201389,(mpz_class)8);
