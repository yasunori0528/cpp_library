...
struct composite{
    ...
    int pqkable(){
        ...
        while(1){
            ...
            for(int j = 13; j >= 12; j--){
                while(vi[j] > 4 && vi[1] < 4 && vi[j%10] < 4){
                    vi[j]--;
                    vi[1]++;
                    vi[j%10]++;
                }
            }
            while(vi[11] > 4 && vi[1] <= 2){
                vi[11]--;
                vi[1] += 2;
            }
...
void calc_cmp1(vector<int> v){
    ...
    MAX_N = min(MAX_N_default, pow_n((mpz_class)10,(mpz_class)(MAX_DIGIT-d)));
    ...
    do{
        ...
        else if(cmp.abs < MAX_CMP.abs) continue;
        else if (MAX_CMP.abs <= cmp.abs && cmp.abs <= MAX_N && cmp.pqkable() <= 2){
            MAX_CMP = cmp;
            //cmp.print();
        }
...
void calc2_sub(power pw){
    for(int i = 1; i <= m-pw.digit; i++){
        mpz_class cmp_upper_bound = pow_n((mpz_class)10,(mpz_class)(72-i-pw.digit));
        for(composite cmp : CMP_list_small[i]){
            cmp.abs *= pw.abs;
            if(cmp < MAX_CMP) continue;
            if(cmp.abs >= cmp_upper_bound) break;
            cmp.p_fact.push_back(pw);
            if (MAX_CMP.abs <= cmp.abs && cmp.abs <= MAX_N && cmp.pqkable() <= 2){
                MAX_CMP = cmp;
                //cmp.print();
            }
        }
...
void calc2(){
    ...
    //n+1桁以上の素数を順に探索
    MAX_N = min(MAX_N_default, pow_n((mpz_class)10,(mpz_class)(72-n-1)));
    if(p_start == 0){
        p_start = pow_n((mpz_class)10,(mpz_class)n)+1;
    }
    for(mpz_class p = p_start; p < pow_n((mpz_class)10,(mpz_class)(m-1)); p += 2){
...
void input(){
    ...
    //m = n;
    MAX_N_default
    = pow_n((mpz_class)10, (mpz_class)69);
    //= pow_n((mpz_class)10, (mpz_class)6);
    MAX_N = MAX_N_default;
    CMP_list_small.resize(n+1);
    p_start = 0;

    MAX_CMP.abs
    // = pow_n((mpz_class)3,(mpz_class)8)*pow_n((mpz_class)201389,(mpz_class)8);
    // = pow_n((mpz_class)10,(mpz_class)46);
    = 1;
    time_interval = 1000*3600;
    return;
}
