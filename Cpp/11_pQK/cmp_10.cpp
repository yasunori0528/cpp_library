...
int main (){
    ...
    //power(41413,7,72)*power(41,7,72)*power(7,3,72);
    //power(5,43,72);//3
    ...
    vector<int> cnt1(72,0);//個数集計
    vector<int> cnt2(72,0);//個数集計
    ...
    for(int64_t i = 0; i < product_pattern; i++){
        ...
        while(bool_calc){
            ...
            if(lower_bound_of_search_range <= calc_t && calc_t < upper_bound_of_search_range){
                string calc_t_str = to_string_Bint(calc_t);
                cnt1[calc_t_str.size()]++;
                if(pqkable2(t+"="+calc_t_str) <= 2){
                    cout << t << "=\n" << calc_t << endl;
                    cnt2[calc_t_str.size()]++;
                    lower_bound_of_search_range = calc_t;
                    //string sss; cin >> sss;
                }
            }
