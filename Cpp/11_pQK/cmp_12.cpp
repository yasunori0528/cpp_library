...
int main(){
    ...
    vector<vector<pair<Bint,Bint>>> vec(25,vector<pair<Bint,Bint>>(0));
    vec[1] = {make_pair(1,1),make_pair(3,9),make_pair(7,49),make_pair(9,81)};
    for(int i = 2; i <= 24; i++){
        rep(j,10){
            Bint _10i = power(10,i);
            Bint j10i = j*power(10,i-1);
            Bint j10isq = j10i*j10i;
            for(pair<Bint,Bint> pr : vec[i-1]){
                Bint pr1 = pr.first;
                Bint pr2 = pr.second;
                Bint k = j10i+pr1;
                Bint ksq = j10isq+2*j10i*pr1+pr2;
                Bint ksq_ = ((2*pr1%10)*j10i+pr2)%_10i;
                string s_ = to_string_Bint(k)+"^2="+to_string_Bint(ksq_);
                if(pqkable2(s_) <= 2){
                    if(i >= 7 && j != 0){
                        string s = to_string_Bint(k)+"^2="+to_string_Bint(ksq);
                        if(pqkable2(s) <= 2) cout << s << endl;
                    }
                    vec[i].push_back(make_pair(k,ksq));
                }
...

        {
            int Time = time(NULL);
            time_count += Time-TIME;
            cout << "time:" << time_count <<  ",end:" << i << ",size:" << vec[i].size() << endl;
        }
    }
