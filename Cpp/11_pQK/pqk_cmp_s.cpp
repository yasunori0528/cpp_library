...
string fact(int64_t n){
    string rtn = to_string(n);
    if(!pt(n)) {
        bool b = false;
        int64_t m = n;
...
int main(){
    ...
    vector<pair<double,string>> output_list;
    for(int i = 1; i <= 13; i++){
        for(int j = 0; j <= 13; j++){
            int n = i;
            if(j < 10) n = 10*n+j;
            ...
            output_list.push_back(make_pair(d,s));
        }
    }
    sort(output_list.begin(),output_list.end());
    reverse(output_list.begin(),output_list.end());

    for(auto pi : output_list){
        string si = pi.second;
        map<char,int> s_map = stom(si);
        vector<char> vc = {'2','3','7'};
        for(char c : vc){
            if(s_map[c]) cout << c;
            else cout << '_';
        }
