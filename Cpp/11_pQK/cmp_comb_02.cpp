...
int main(){
    ...
    int deck_d_size = 0;
    for(char c : deck){
        if('0' <= c && c <= '9') deck_d_size++;
        else deck_d_size += 2;
    }
    int64_t maxp = pow((int64_t)10,(deck_d_size-1)/4+1);
    //= pow(10,8);
    prime_calculate(maxp);
    vector<vector<tuple<int,int64_t,string>>> output_list(deck.size()+1,vector<tuple<int,int64_t,string>>(0));
    for(int d = 2; d <= deck_d_size-1; d++){

        Bint max_cmp = pow((Bint)10,deck_d_size-d)-1;
        Bint min_cmp = 
        1;
        ...
        //cout << "exp_pattern" << endl;

        vector<vector<pair<Bint,string>>> exp_list(d+1,vector<pair<Bint,string>>(0,make_pair(0,"")));
        int max_exp_p = max_exp;
        for(int64_t p = 2; p < pow((int64_t)10,(deck_d_size-1)/2); p++){
            if(!pt(p)) continue;
            //cout << "s" << p << endl;
            string p_str = to_string(p);
            int p_size = p_str.size();
            if(p_size >= d || p_size > deck_d_size-d) break;
            if(pqkable3(p_str, deck)) exp_list[p_size].push_back(make_pair(p,p_str));
            else continue;
            //cout << "1" << p << endl;
            Bint pow_p_i = p;
            for(int i = 2; i <= max_exp_p; i++){
                ...
                for(string s : exp_pattern[i]){
                    ...
                    if(pow_p_i_size <= d && pqkable3(pow_p_i_str,deck)){
                ...
                if(cmp < min_cmp){
                    ...
                    if(cmp > max_cmp){
                        ...
                        if(pqkable3(s,deck)){
                            ...
                            int64_t cmp_int = str_int(ss.substr(0,d_c));

                            output_list[d_s].push_back(make_tuple(d_c,cmp_int,ss));
                            cnt++;
                            //cnt1[cmp_str.size()]++;
                            //min_cmp = cmp;
                            //if(d_s >= 19) cout << s << endl;
                        }
                        update_loop = d_vec_size-1;
    ...
    for(vector<tuple<int,int64_t,string>> &vec : output_list){
    ...
    for(int i = 3; i <= deck.size(); i++){
        cout << "---d=" << i << "---" << endl;
        for(tuple<int,int64_t,string> t : output_list[i]){
            /*何らかの条件
            int s_d = 0;
            for(char c : s){
                if(c == '=') break;
